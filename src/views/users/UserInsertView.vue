...
/**
 * API 명세(제공된 코드에 근거)
 * - 공통코드(권한코드): GET /cmmn-management/code/AA → Envelope<List<CommonCodeVO>>{data:[{cmmnCd,cmmnNm,...}]}
 ...
 *
 * 주의:
 * - 이미지 업로드 REST 엔드포인트 명세 없음 → 미리보기만 제공, 전송은 보류.
 * - CSRF 헤더/쿠키는 axios 인터셉터에서 처리한다고 가정.
 */
...
const form = ref({
  ...
  // profileGrpId: null // 파일 업로드 엔드포인트 제공 시 채우기
})
...
async function onSubmit() {
  ...
  try {
    ...
    // 2) 권한 병합 (username 쿼리는 서비스 예시대로 null 허용 가정)
